...
RSpec.describe 'GetCategory integration test' do
  it 'returns a category with corresponding id' do
    ...
    request_dto = Application::UseCase::GetCategoryRequest.new(id: movie_category.id)

    response_dto = use_case.execute(request_dto)

    expect(response_dto)
      .to(eq(
            Application::UseCase::GetCategoryResponse.new(
  ...
  it 'raises error when category is not found' do
    ...
    request_dto = Application::UseCase::GetCategoryRequest.new(id: not_found_id)
